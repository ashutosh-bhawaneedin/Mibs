...
# **Mibs 🦍** [![AGPL License](https://img.shields.io/badge/license-AGPL-blue.svg)](http://www.gnu.org/licenses/agpl-3.0)
Mibs is a Free and Open Source HRMS Software.

<img width="1470" alt="Screenshot 2024-03-15 at 3 05 20 PM" src="https://github.com/Mibs-opensource/Mibs/assets/131998600/1317bd0a-03a8-40be-8fb2-ecb655bb5c13">


## **Installation**
____
Mibs can be installed on your system by following the below commands.
...
For installing the Mibs, follow the following steps:
1. Clone the project repository from GitHub:
```bash
git clone https://github.com/Mibs-opensource/Mibs.git
```
...
3. Set up the database by running the following commands:
   _By default the test database will be loaded which will have demo data inside it. If you wish to start with a fresh database, you can either remove the TestDB_Mibs.sqlite3 from the project directory or change the name of the database inside the Mibs/settings.py file. (You can configure different database based on your choice, of which configurations settings is given below in the documentation._
```bash
python manage.py makemigrations
python manage.py migrate
```
4. Create an admin employee account (use this command if you are starting with a fresh database, for the demo database there is already a Mibs admin user created with credentials _admin_ and _admin_ as username and password respectively).
```bash
python manage.py createMibsuser
```
>Note: createMibsuser is a similar command to createsuperuser in Django,  which creates an admin user along with a related admin employee into the database.
...
```
If everything is configured correctly, you should be able to access your Mibs app at http://localhost:8000.

>Note: 
>>If you wish to run the Mibs application to any other port, you can specify the port number after the runserver command.
...
>>If you wish to start with a fresh database, remove the db.sqlite3 file from the project directory and run the migrate command followed by the createMibsuser command to start with a fresh database.
